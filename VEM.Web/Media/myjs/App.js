...
var GetThisMenu = function () {
    return {
        init: function () {
            var url = window.location.toString()
            var arr = new Array();
            arr = url.split('/');
            var controlleName = arr[3];
            var actionName = arr[arr.length - 1];
            var arrhref = $(".page-sidebar-menu a");
            var thisid = new String();
            $.each(arrhref, function () {
                var temp = new Array();
                var idtemp = this.id.toString();
                temp = this.href.split('/');
                if (controlleName == temp[3]) {
                    var urr = new Array();
                    urr = temp[4].split('?');
                    if (actionName.indexOf(urr[0]) >= 0) {
                        thisid = idtemp.toString();
                    }
                }
            });
            $("#" + thisid).parent().attr("class", "active");
            $("#" + thisid).parent().parent().parent().attr("class", "active");
            if (controlleName == "Home" || controlleName == "") {
                $("#homeli").attr("class", "active");
            } else {

                $("#homeli").attr("class", "start");
            }
        }
